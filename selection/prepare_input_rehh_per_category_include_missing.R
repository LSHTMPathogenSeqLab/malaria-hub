...
# Count Ref, Alt, NA and 0.5 for checking purpose (differances when changing ref to 0 i.e.) 
fws_samples <- metadata %>% filter(fws > threshold_fws)
levels <- unique(do.call(c, snp %>% dplyr::select(as.vector(fws_samples$Sample_bqsr))))
out <- sapply(levels, function(x) rowSums(snp %>% dplyr::select(as.vector(fws_samples$Sample_bqsr)) ==x))
snp_count <- cbind(snp %>% dplyr::select(chr, pos, ref), out)

write.table(snp_count,
